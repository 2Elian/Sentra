...
from sentra.core.agents.gqag.generator import AggregatedGenerator, MultiHopGenerator, CoTGenerator
...
class GenerateService:
    ...
    def __init__(self,llm_sentra: BaseLLMClient = None):
        self.llm_client = llm_sentra
        self.method = settings.agents.gqag.method
        self.data_format = settings.agents.gqag.data_format
    ...
    async def build(self, namespace, kb_id) -> Tuple[List[Dict[str, Any]], List[Dict[str, Any]], List[Dict[str, Any]]]:
        batches, batches_leiden = await self.subgraph_builder(namespace, kb_id)
        aggregated_task = run_concurrent(
            self.generator_aggregated.generate,
            batches,
            desc="sentra.gqag: Generating QAs for aggregated type ",
            unit="batch_aggregated",
        )
        multihop_task = run_concurrent(
            self.generator_multihop.generate,
            batches_leiden,
            desc="sentra.gqag: Generating QAs for multi_hop type ",
            unit="batch_multi-hop",
        )
        cot_task = run_concurrent(
            self.generator_cot.generate,
            batches_leiden,
            desc="sentra.gqag: Generating QAs for cot type ",
            unit="batch_cot",
        )
