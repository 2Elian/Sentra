...
class KgBuilder:
    ...
    def __init__(self, llm_sentra: BaseLLMClient = None):
        self.logger = knowledgeBase_logger
        self.llm_sentra = llm_sentra
        self.max_loop = settings.kg.max_loop

    # local_perception_recognition
    async def local_perception_recognition(self, chunk: Chunk, entity_types, entity_types_des) -> Tuple[
        ...
        try:
            ...
            hint_prompt = KG_EXTRACTION_PROMPT[language]["TEMPLATE"].format(
                **KG_EXTRACTION_PROMPT["FORMAT"], input_text=contract_content,
                entity_types=entity_types, entity_types_des=entity_types_des)
            hint_prompt_token_count = self.llm_sentra.tokenizer.count_tokens(hint_prompt)
            self.logger.debug(f"the hint prompt token size: \n {hint_prompt_token_count}")
            # initial glean
            final_result = await self.llm_sentra.generate_answer(hint_prompt)
            ...
            for loop_idx in range(self.max_loop):
                if_loop_result = await self.llm_sentra.generate_answer(
                ...
                glean_result = await self.llm_sentra.generate_answer(
    ...
    async def merge_nodes(
        ...
        description = await self._handle_kg_summary(entity_name, description,
                                                    max_summary_tokens=settings.kg.max_summary_tokens)
    ...
    async def _handle_kg_summary(
        ...
        tokenizer_instance = self.llm_sentra.tokenizer
        ...
        new_description = await self.llm_sentra.generate_answer(prompt)
        self.logger.info(
